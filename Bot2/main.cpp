...
//-------------------------------------------------------------------------------------------
  ...
  while (1) {
      if (currState == state1) {
          ...
          // Letting partner team know we found the red lane
          client.beginMessage(TYPE_TEXT);
          ...
          // Trying to see if we can proceed to Yellow Lane
          while(!proceedYellow) {
            while ((client.parseMessage()) > 0) {
              String message = client.readString();
          ...
          // Letting partner team know we got home
          client.beginMessage(TYPE_TEXT);
          client.print("Back home");
          client.endMessage();
