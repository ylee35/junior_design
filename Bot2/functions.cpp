...
void laneFollowing(int targetColor) {
    ...
    BotMotions motions;
    motions.set_speeds(125,125,125);

    const int STEP_MS   = 200;   // small action step
    //200
        ...
        switch (laneState) {
            case ON_COLOR: {
                if (colors[RIGHT] == targetColor) {
                    ...
                    delay(100);
                    laneState = ON_COLOR;
                } else if (colors[RIGHT] == BLACK && colors[LEFT] == BLACK) {
                    motions.stop();
                    delay(100);
                    laneState = SEARCH_RIGHT_1;
                } else if (colors[RIGHT] == YELLOW) {
                    motions.pivot_cc();
                    delay(STEP_MS);
                    motions.stop();
...
void laneFollowingStraight(int targetColor) {
    ...
    while (true) {
        ...
        int *colors = colorSensed();
        bool onTarget = ((colors[RIGHT] == targetColor) || (colors[LEFT] == targetColor));
        switch (laneState) {
            case ON_COLOR: {
                if ((colors[RIGHT] == targetColor) || (colors[LEFT] == targetColor)) {
                    // pulse forward, then re-check next loop
                    Serial.println("on target");
                    motions.forward();
                    delay(FWD_MS);
